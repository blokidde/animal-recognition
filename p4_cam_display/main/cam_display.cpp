// ESP-IDF (C++) — GC9A01 240x240 SPI (Waveshare 1.28")
// Vendor-driver gebruikt: esp_lcd_new_panel_gc9a01 (stabiel, volledige init)
...
static constexpr uint32_t LCD_SPI_HZ = (60u * 1000u * 1000u);  // 60 MHz = stabiel
...
static int measure_text_px(const char* s, int scale) { int w = 0; for (const char* p = s; *p; ++p) w += char_width(*p, scale); return w; }
                                  ...
                                  std::vector<uint16_t>& buf, int& out_w, int& out_h)
{
  ...
  for (const char* p = s; *p; ++p) {
    if (*p >= '0' && *p <= '9') {
      ...
      for (int dy = 0; dy < scale; ++dy) buf[((6*scale) + dy) * out_w + cx] = fg;
      cx += (1 + 1) * scale;
    } else if (*p == ',') {
...
extern "C" void app_main(void)
{
  ...
  buscfg.max_transfer_sz = LCD_HRES * STRIPE_H * (int)sizeof(uint16_t);
  ...
  // Panel (vendor driver met volledige init)
  ...
  // Als kleuren off lijken, probeer inversion aan:
  ...
  const int SCREEN_W = LCD_HRES;
  const int SCREEN_H = LCD_VRES;

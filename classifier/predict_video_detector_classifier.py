...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--cls-conf", type=float, default=0.0, help="Only draw classifier predictions above this threshold")
...
def classify_crop(
    ...
    if crop.size == 0:
        return StablePrediction(0, class_names[0], 0.0)
    ...
    return StablePrediction(idx, class_names[idx], float(confidence.item()))
...
def main() -> None:
    ...
    stable_by_track: dict[int, StablePrediction] = {}
        ...
        while True:
            ...
            if boxes is not None and boxes.id is not None:
                ...
                for coords, track_id in zip(xyxy, track_ids):
                    ...
                    should_classify = (
                        track_id not in stable_by_track
                        or frame_index - last_classified_frame.get(track_id, -10**9) >= max(1, args.classify_every)
                    )
                    if should_classify:
                        prediction = classify_crop(frame, crop_box, classifier, transform, class_names, device)
                        previous = stable_by_track.get(track_id)
                        if previous is None or prediction.confidence > previous.confidence:
                            stable_by_track[track_id] = prediction
                        last_classified_frame[track_id] = frame_index
                    stable = stable_by_track.get(track_id)
                    if stable is not None and stable.confidence >= args.cls_conf:
                        label = f"{stable.class_name} {stable.confidence:.2f} id:{track_id}"
                        draw_label(frame, raw_box, label, track_color(track_id))
                    handled += 1
            writer.write(frame)
